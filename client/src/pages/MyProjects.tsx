    ...
    {loading ? (
      ...
      <div className="py-10 min-h-[80vh]">
        <div className="flex items-center justify-between mb-12">
          ...
          <button
            onClick={() => navigate("/")}
            className="flex items-center gap-2 px-4 sm:px-6 py-2 rounded-md text-white active:scale-95 transition bg-gradient-to-r from-[#3AA9FF] via-[#6A6CFF] to-[#FF5DB1] hover:shadow-[0_0_25px_rgba(255,93,177,0.35)]"
          >
            <PlusIcon size={18} />
            Create New
          </button>
        ...
        <div className="flex flex-wrap gap-4">
          {projects.map((project) => (
            <div
              ...
              className="relative group w-72 max-sm:mx-auto cursor-pointer rounded-xl overflow-hidden border border-white/10 bg-white/5 backdrop-blur hover:border-[#6A6CFF]/60 hover:shadow-[0_0_30px_rgba(106,108,255,0.25)] transition-all"
            >
              ...
              <div onClick={(e) => e.stopPropagation()}>
                <TrashIcon
                  ...
                  className="absolute top-3 right-3 scale-0 group-hover:scale-100 size-7 p-1.5 rounded-md bg-white text-red-500 cursor-pointer transition"
                />
              </div>
            </div>
          ))}
